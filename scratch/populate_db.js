...
// Define the 5 curated questions with robust templates and test runners directly in JS
...
async function bootstrap() {
  try {
    const jsonPath = path.join(__dirname, '..', 'parsed_questions.json');
    if (!fs.existsSync(jsonPath)) {
      console.error('Missing parsed_questions.json at', jsonPath);
      process.exit(1);
    }
    ...
    // Merge 5 curated questions
    const merged = parsed.map((pq) => {
      const curated = CURATED_QUESTIONS.find(
        (cq) => cq.id === pq.id || cq.title.toLowerCase() === pq.title.toLowerCase()
      );
      if (curated) {
        console.log(`Blending curated starter code/test runner for "${pq.title}"`);
        return {
          ...pq,
    ...
    console.log('Mapping questions to DB row format...');
    const dbRows = merged.map((q) => ({
      id: q.id,
      title: q.title,
    ...
    console.log(`Importing ${dbRows.length} questions into Supabase 'leetcode_questions' table...`);
    ...
    for (let i = 0; i < dbRows.length; i += chunkSize) {
      ...
      if (error) {
        throw new Error(`Failed at chunk index ${i}: ${error.message}`);
      }
      console.log(`Upserted chunk starting at ${i}...`);
    }
